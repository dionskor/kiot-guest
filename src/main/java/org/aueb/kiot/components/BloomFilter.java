...
public class BloomFilter<E> implements Serializable {
    ...
    private static BloomFilter BF_catalog;
    private static List<BloomFilter> BF_tags;
     ...
     * (maximum number of elements that will be used)
    ...
    /**
     * Get BF_catalog of node. If the type of node is 3 then set BF_catalog with
     * path and tags. Any other type of node has an empty BloomFilter.
     *
     * @return BF_catalog
     */
    public static BloomFilter getBF_catalog() {
        if (BF_catalog == null) {
            BF_catalog = new BloomFilter(Configuration.BF_SIZE, Configuration.MAX_NUM_TAGS);
            BF_catalog.addAll(Configuration.tags);
        }
        return BF_catalog;
    }

    /**
     * get BF_tags only for leaf nodes (Type = 3)
     *
     * @return BF_tags
     */
//    public static List <BloomFilter> getBF_tags() {
//        if (BF_tags == null) {
//            BF_tags = Collections.synchronizedList(new ArrayList<BloomFilter>());
//            for (String tag : Configuration.tags) {
//                BloomFilter bf_tag = new BloomFilter(Configuration.BF_SIZE, Configuration.MAX_NUM_TAGS);
//                bf_tag.addAll(Configuration.path);
//                bf_tag.add(tag);
//                BF_tags.add(bf_tag);
//            }
//        }
//        return BF_tags;
//    }
    public static void setBF_catalog(BloomFilter bf) {
        BF_catalog = bf;
    }
    ...
    /**
     * Construct a new Bloom filter based on existing Bloom filter data without
     * the ability to count the FalsePositive probability.
     *
     * @param bitSetSize defines how many bits should be used for the filter.
     * @param expectedNumberOfFilterElements defines the maximum number of
     * elements the filter is expected to contain.
     * @param filterData a BitSet representing an existing Bloom filter.
     */
    public BloomFilter(int bitSetSize, int expectedNumberOfFilterElements, BitSet filterData) {
        this(bitSetSize, expectedNumberOfFilterElements);
        this.bitset = filterData;
    }
    ...
    /**
     * Returns true if the specified bloom filter is contained in this.
     *
     * @param bf bloom filter to check
     * @return true if the specified bloom filter is contained in this
     */
    public boolean contains(BloomFilter bf) {
        BitSet bs1 = this.bitset;
        BitSet bs2 = bf.getBitSet();
        for (int i = 0; i < bs1.size(); i++) {
            if (bs2.get(i)) {
                if (bs2.get(i) == bs1.get(i)) {
                    continue;
                }
                return false;
            }
    ...
    /**
     * Returns the union of BloomFilter with the specified one.
     *
     * @param bf BloomFilter for union
     * @return union of bloom filters
     */
    public BloomFilter union(BloomFilter bf) {
        BitSet bits = (BitSet) this.bitset.clone();
        BloomFilter cloneBf = new BloomFilter(this.bitSetSize, this.expectedNumberOfFilterElements, this.numberOfAddedElements, this.bitset);
        cloneBf.getBitSet().or(bf.getBitSet());
        return cloneBf;
    }
